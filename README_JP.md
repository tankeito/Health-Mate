...
cron や最小 PATH の shell 環境では、`openclaw` が見つからずローカル LLM がスキップされることがあります。`OPENCLAW_BIN` を設定しておくと、日次ランナーと Python 側の両方で優先的に利用されます。
...
```bash
python scripts/export_memory_jp.py
python scripts/daily_health_report_pro.py memory_jp/2026-03-20.md 2026-03-20
python scripts/weekly_report_pro.py 2026-03-20
python scripts/monthly_report_pro.py 2026-03-20
```
