...
- PDF の視覚的スタイルを強化し、レイアウトと可読性を向上
- 日次・週次・月次レポートジェネレーターの軽微な書式問題を修正
- シンボルレンダリングを最適化し、クロスプラットフォーム互換性を向上

### v1.5.3 - 2026-03-29

- ローカル LLM 出力をサニタイズし、AI コメントがプッシュテキストや PDF に埋め込まれる際のログ漏洩を防止
- 予期せぬプラグインログ断片に対する防御的フォールバックとして、2 つ目の日次 PDF コメントフィルターを追加
- プロジェクトローカルの config/.env を自動読み込みすることで、直接 Python 実行をシェルランナーと整合
