...
如果你的定时任务环境没有继承交互式 shell 里的 Node / NVM `PATH`，请在 `config/.env` 中额外设置 `OPENCLAW_BIN`。当前 shell 入口和 Python 控制器都会优先使用它来定位本地 `openclaw`。
...
- 🔧 将 `NVM_DIR` 和 `CRON_PATH` 移至 `.env` 和 `.env.example` 文件集中管理
- 📝 更新 `daily_health_report_pro.sh`、`weekly_health_report_pro.sh` 和 `monthly_health_report_pro.sh` 从 `.env` 加载环境变量
