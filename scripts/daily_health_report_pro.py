...
def default_module_title(locale, module_id):
    ...
    extra_titles = {
        "medication": {"zh-CN": "用药情况", "en-US": "Medication"},
    }
    locale = resolve_locale(locale=locale)
    return extra_titles.get(module_id, {}).get(locale, module_id)


def localize_profile_item(locale, value):
    text = str(value or "").strip()
    if not text or resolve_locale(locale=locale) != "en-US":
        return text
...
def generation_source_label(locale, source):
    locale = resolve_locale(locale=locale)
    labels = {
        "llm": {"zh-CN": "\u6765\u6e90\uff1aLLM \u52a8\u6001\u751f\u6210", "en-US": "Source: dynamically generated by LLM"},
        "fallback": {"zh-CN": "\u6765\u6e90\uff1a\u672c\u5730\u89c4\u5219", "en-US": "Source: local rules"},
        "fallback_tavily": {"zh-CN": "\u6765\u6e90\uff1aTavily \u68c0\u7d22 + \u672c\u5730\u89c4\u5219", "en-US": "Source: Tavily retrieval + local rules"},
        "local": {"zh-CN": "\u6765\u6e90\uff1a\u672c\u5730\u89c4\u5219", "en-US": "Source: local rules"},
    }
    return labels.get(source, labels["local"]).get(locale, labels["local"]["en-US"])
...
def infer_memory_locale_from_text(content):
    ...
    if en_score >= max(2, zh_score + 1):
        return "en-US"
    if zh_score >= max(2, en_score + 1):
        return "zh-CN"
...
def build_multi_condition_tip(locale, conditions, standards):
    ...
    joiner = "；" if resolve_locale(locale=locale) == "zh-CN" else "; "
    ...
    for match in level3_blocks:
        ...
        if exercise_type:
            distance_match = re.search(
                r'(?:距离|路程|distance)\s*[:：]\s*([\d.]+)\s*(?:公里|km)',
                block_text,
                re.IGNORECASE,
            )
...
def run_local_llm(prompt, system_prompt, settings, locale, timeout_key, failure_key):
    ...
    if mode == "local_only" or not shutil.which("openclaw"):
        return None
...
def generate_ai_plan(health_data, config):
    ...
    fruit_text = favorite_fruits[0] if favorite_fruits else ("苹果" if locale == "zh-CN" else "apple")
    avoid_foods = localized_dislike + localized_allergies
    avoid_text = "、".join(avoid_foods[:2]) if locale == "zh-CN" else ", ".join(avoid_foods[:2])
    water_split = [max(200, round(target_water * ratio / 100 / 50) * 50) for ratio in [15, 20, 25, 20, 20]]
    breakfast_base = "燕麦粥 + 水煮蛋 + " + fruit_text if locale == "zh-CN" else f"oatmeal + boiled egg + {fruit_text}"
    lunch_base = "杂粮饭 + 鸡胸肉 + 两份蔬菜" if locale == "zh-CN" else "mixed grains + chicken breast + two servings of vegetables"
    dinner_base = "豆腐/鱼肉 + 绿叶菜 + 少量主食" if locale == "zh-CN" else "tofu or fish + leafy vegetables + a small portion of carbs"
    if t(locale, "shortcoming_fat_low") in shortcomings:
        breakfast_base += " + 一小把坚果" if locale == "zh-CN" else " + a small handful of nuts"
    if t(locale, "shortcoming_fat_high") in shortcomings:
        lunch_base = "清蒸蛋白质 + 焯蔬菜 + 少油主食" if locale == "zh-CN" else "lean steamed protein + blanched vegetables + low-oil carbs"
    if t(locale, "shortcoming_fiber_low") in shortcomings:
        dinner_base += "，并增加豆类或菌菇" if locale == "zh-CN" else ", plus beans or mushrooms for extra fiber"
    if avoid_text:
        dinner_base += f"（避开 {avoid_text}）" if locale == "zh-CN" else f" (avoid {avoid_text})"

    plan = {
        "diet": [
            {"time": "07:30-08:30", "meal": "早餐" if locale == "zh-CN" else "Breakfast", "menu": breakfast_base, "calories": 320, "fat": 8 if t(locale, "shortcoming_fat_low") in shortcomings else 6, "fiber": 7},
            {"time": "12:00-13:00", "meal": "午餐" if locale == "zh-CN" else "Lunch", "menu": lunch_base, "calories": 480, "fat": 10 if t(locale, "shortcoming_fat_high") not in shortcomings else 7, "fiber": 8},
            {"time": "18:00-19:00", "meal": "晚餐" if locale == "zh-CN" else "Dinner", "menu": dinner_base, "calories": 420, "fat": 9 if t(locale, "shortcoming_fat_low") in shortcomings else 6, "fiber": 9 if t(locale, "shortcoming_fiber_low") in shortcomings else 6},
        ],
        "water": [
            {"time": "07:00-08:00", "amount": f"{water_split[0]}ml", "note": "晨起先补水" if locale == "zh-CN" else "Hydrate after waking"},
            {"time": "10:00-11:00", "amount": f"{water_split[1]}ml", "note": "上午工作间隙" if locale == "zh-CN" else "Mid-morning refill"},
            {"time": "13:30-14:30", "amount": f"{water_split[2]}ml", "note": "午后补水" if locale == "zh-CN" else "Afternoon hydration"},
            {"time": "17:00-18:00", "amount": f"{water_split[3]}ml", "note": "下班前完成一轮" if locale == "zh-CN" else "Finish another round before evening"},
            {"time": "20:00-21:00", "amount": f"{water_split[4]}ml", "note": "睡前不过量" if locale == "zh-CN" else "Top up without overdrinking before bed"},
        ],
        "exercise": [
            {
                "time": "晚饭后" if locale == "zh-CN" else "After dinner",
                "activity": "快走" if locale == "zh-CN" else "Brisk walk",
                "duration": "20-30分钟" if locale == "zh-CN" else "20-30 minutes",
                "details": "目标补足步数并帮助消化" if locale == "zh-CN" else "Use it to recover steps and support digestion",
            }
        ],
        "notes": [
            ("今日重点围绕 " if locale == "zh-CN" else "Tomorrow should focus on ")
            + condition_text
        ],
    }
...
def build_module_detail_text(locale, module_id, info):
    ...
    return "\u672a\u8bb0\u5f55" if locale == "zh-CN" else "Not recorded"
...
def generate_risk_alerts(health_data, config):
    ...
    if fat_val > fat_max:
        risks.append({
            'level': localize('\u9ad8', 'High'),
            'item': localize('\u8102\u80aa\u6444\u5165\u504f\u9ad8', 'Fat intake high'),
            'risk': localize(
                f"\u4eca\u65e5\u8102\u80aa {fat_val:.1f}g\uff0c\u9ad8\u4e8e\u5efa\u8bae\u4e0a\u9650 {fat_max}g\u3002",
                f"Fat reached {fat_val:.1f}g today, above the suggested ceiling of {fat_max}g.",
            ),
            'action': localize('\u660e\u65e5\u51cf\u5c11\u6cb9\u70b8\u3001\u80a5\u8089\u3001\u5976\u6cb9\u548c\u91cd\u6cb9\u9171\u6c41\u3002', 'Cut fried food, fatty meat, cream, and oily sauces tomorrow.'),
        })
    elif fat_val < fat_min * 0.7:
        risks.append({
            'level': localize('\u4e2d', 'Medium'),
            'item': localize('\u8102\u80aa\u6444\u5165\u504f\u4f4e', 'Fat intake low'),
            'risk': localize(
                f"\u4eca\u65e5\u8102\u80aa\u4ec5 {fat_val:.1f}g\uff0c\u660e\u663e\u4f4e\u4e8e\u5efa\u8bae\u4e0b\u9650 {fat_min}g\u3002",
                f"Fat was only {fat_val:.1f}g today, well below the suggested floor of {fat_min}g.",
            ),
            'action': localize('\u53ef\u9002\u91cf\u8865\u5145\u575a\u679c\u3001\u725b\u6cb9\u679c\u6216\u6a44\u6984\u6cb9\u7b49\u5065\u5eb7\u8102\u80aa\u3002', 'Add a moderate amount of healthy fats such as nuts, avocado, or olive oil.'),
        })
    ...
    if fiber_val < fiber_min:
        risks.append({
            'level': localize('\u4e2d', 'Medium'),
            'item': localize('\u81b3\u98df\u7ea4\u7ef4\u4e0d\u8db3', 'Fiber intake low'),
            'risk': localize(
                f"\u4eca\u65e5\u81b3\u98df\u7ea4\u7ef4 {fiber_val:.1f}g\uff0c\u4f4e\u4e8e\u5efa\u8bae\u503c {fiber_min}g\u3002",
                f"Fiber was {fiber_val:.1f}g today, below the recommended {fiber_min}g.",
            ),
            'action': localize('\u660e\u65e5\u589e\u52a0\u6df1\u8272\u852c\u83dc\u3001\u8c46\u7c7b\u548c\u7c97\u7cae\u3002', 'Increase vegetables, beans, and whole grains tomorrow.'),
        })
    ...
    if water_val < water_target * 0.75:
        risks.append({
            'level': localize('\u4e2d', 'Medium'),
            'item': localize('\u996e\u6c34\u4e0d\u8db3', 'Hydration low'),
            'risk': localize(
                f"\u4eca\u65e5\u996e\u6c34 {water_val}ml\uff0c\u4f4e\u4e8e\u76ee\u6807 {water_target}ml \u7684 75%\u3002",
                f"Hydration was {water_val}ml today, below 75% of the {water_target}ml target.",
            ),
            'action': localize('\u628a\u996e\u6c34\u62c6\u6210 4-6 \u6b21\u5b8c\u6210\uff0c\u4f18\u5148\u5b89\u6392\u6668\u8d77\u548c\u5348\u540e\u8865\u6c34\u3002', 'Split water into 4-6 sessions, especially after waking and in the afternoon.'),
        })
    ...
    if symptoms:
        ...
        risks.append({
            'level': localize('\u9ad8', 'High'),
            'item': localize('\u75c7\u72b6\u8bb0\u5f55\u5f02\u5e38', 'Symptoms recorded'),
            'risk': localize(
                f"\u4eca\u65e5\u8bb0\u5f55\u5230\u75c7\u72b6\uff1a{symptom_text}\u3002",
                f"Symptoms were recorded today: {symptom_text}.",
            ),
            'action': localize('\u82e5\u6301\u7eed\u6216\u52a0\u91cd\uff0c\u8bf7\u4f18\u5148\u4f11\u606f\u5e76\u8003\u8651\u7ebf\u4e0b\u5c31\u533b\u3002', 'If symptoms persist or worsen, rest first and consider offline medical care.'),
        })

    if health_data.get('steps', 0) < 3000 and not health_data.get('exercise_records'):
        risks.append({
            'level': localize('\u4e2d', 'Medium'),
            'item': localize('\u6d3b\u52a8\u91cf\u504f\u4f4e', 'Activity low'),
            'risk': localize(
            ...
            'action': localize('\u660e\u65e5\u5b89\u6392\u4e00\u6b21 15-20 \u5206\u949f\u8f7b\u4e2d\u5ea6\u6b65\u884c\u6216\u9a91\u884c\u3002', 'Plan one 15-20 minute light-to-moderate walk or ride tomorrow.'),
        })

    if module_is_enabled(config, 'medication') and not health_data.get('medication_records'):
        risks.append({
            'level': localize('\u4e2d', 'Medium'),
            'item': localize('\u7f3a\u5c11\u7528\u836f\u8bb0\u5f55', 'Medication record missing'),
            'risk': localize('\u5f53\u524d\u5df2\u542f\u7528\u7528\u836f\u8ffd\u8e2a\uff0c\u4f46\u4eca\u65e5\u6ca1\u6709\u8bb0\u5f55\u3002', 'Medication tracking is enabled, but no medication entry was recorded today.'),
            'action': localize('\u5982\u4eca\u65e5\u6709\u670d\u836f\uff0c\u8bf7\u8865\u5145\u65f6\u95f4\u3001\u836f\u540d\u548c\u5242\u91cf\u3002', 'If medication was taken today, add time, medicine name, and dosage.'),
        })

    if len(conditions) > 1:
        risks.append({
            'level': localize('\u63d0\u793a', 'Info'),
            'item': localize('\u591a\u75c5\u79cd\u8054\u5408\u7ba1\u7406', 'Multi-condition management'),
            'risk': localize(
...
def render_risk_text(risks, locale):
    ...
    lines = []
    for risk in risks:
        advice_prefix = '\u5efa\u8bae' if resolve_locale(locale=locale) == 'zh-CN' else 'Action'
